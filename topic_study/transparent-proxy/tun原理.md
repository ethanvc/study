...
graph TD
    ...
    A -->|"mihomo NAT 改写"| B
    B["改写后\nsrc=198.18.0.2:50000\ndst=198.18.0.1:10000"]
    B -->|"写回 utun，内核收到"| C["内核 TCP 三次握手\n198.18.0.1:10000 = mihomo listenFd"]
    C --> D["accept() 得到 connFd\npeer port = 50000"]
    D -->|"查 NAT 表 nat_table[50000]"| E["恢复原始目标\n93.184.216.34:443"]
```
...
graph TD
    ...
    subgraph "绑定 en0 ✓"
        B1["outFd.connect(代理服务器)"] -->|"IP_BOUND_IF=en0\n绕过路由表"| B2["en0 物理网卡"]
        B2 --> B3["远程服务器 ✓"]
    end
```
...
graph TD
    ...
    B["内核路由表\n0.0.0.0/1 | 128.0.0.0/1 → utun"] --> C
    C["utun 设备\nrecvmsg_x 批量收包\n格式: 4B AF头 | IP头 | TCP/UDP头 | payload"] --> D
    ...
    E["代理隧道\nDNS 劫持 · 进程识别 · 规则匹配"]
    E --> F["outFd\nsetsockopt IP_BOUND_IF=en0 防回环"]
