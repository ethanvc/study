...
export default function EnableProxySwitch({ enabled, onToggle }: EnableProxySwitchProps) {
    return (
        <div className="flex items-center justify-between gap-4 p-3 mb-2 bg-card border border-border rounded-lg">
            ...
            <button
                ...
                onClick={onToggle}
                className={`relative w-10 h-6 rounded-full flex-shrink-0 transition-colors ${enabled ? 'bg-primary' : 'bg-secondary'
                    }`}
            >
                <span
